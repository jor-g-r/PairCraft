...
**Status (2026-07-05, EOD):** Reactivated after a deliberate pause (2026-06-18 → 2026-07-05). Live: search-first home, visual system, market-gated feed across **7 markets** (CO/VE/CL/AR + ES/FR/AE), market-scout pipeline (CO/CL/AR catalogs scraped; ~7100 candidates on file). **Corpus expanded 11→19 wines / 126→218 pairings (scout-driven selection) — sits UNCOMMITTED pending Jorge's voice review** (the May-14 pattern: sample-read, re-roll anything off-voice, then commit). The **Day-21 demo for Teo + profesora has NOT happened** — original target 2026-06-02 lapsed, not cancelled; it remains the next milestone and the gate for tier-engine tuning. Canonical plan is `paircraft-mvp-v2.md` (v1 superseded).
